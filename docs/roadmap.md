...
- 当前阶段：`Iteration 17: Inner Coding Agent Lifecycle` 的 Slice 17.1 已完成实现、review 修复、OpenSpec 归档和规格同步；目标是让 Coordinator 通过 SDK-backed inner coding agent 生成 workflow gate evidence，并在 agent 停止后结合 workflow protocol status 判断下一步。
- 当前 OpenSpec change：`add-inner-coding-agent-lifecycle` 已归档到 `openspec/changes/archive/2026-05-26-add-inner-coding-agent-lifecycle`；上一轮 `add-sdk-backed-gate-evidence` 已完成并提交。
- 当前正式规格：本轮已同步 `agent-provider-runtime`、`daemon-runtime`、`observability`、`web-human-review-surface` 和 `workflow-gate-evidence`；上一轮已同步 `core-data-model`、`coordinator-surface`、`operator-task-controls`、`workflow-protocol-adapter` 等相关规格。
...
- 当前 Slice 14.1 已落地：共享层新增 workflow action classification；Core operator-facing workflow action helper 仅允许 `freeze-requirements`、`approve-planning-dossier`、review/merge approval 类 gate；API `POST /workflow-runs/:id/actions` 继续只进 Core helper，并拒绝 `materialize-change`、agent/internal 和 unknown/debug-only action；旧 Web Task Cockpit / Workbench / Action Inbox / needs-me 只展示 operator-facing workflow gate；Workflow Lens debug/detail 仍展示 internal/debug action 和 action input hint；`Run until blocked` 对 internal/debug action 显示 observing，不把开发者拉进手动参数输入。
...
- 当前 Slice 15.2 已落地：Web V2 使用单一 `Needs-Me Gate Inbox` projection 驱动 Command Bar count、Run Matrix pin 和 Focus Drawer gate card；pending human request、merge approval、PR/MR review/conflict、operator-facing workflow gate、Core recovery attention 和 failed/unknown/blocked high-risk state 才进入 needs-me；`materialize-change`、`run-alignment-checks`、inspect/resume、unknown/debug action 只进入 Workflow Lens / Debug Detail。`Run until blocked` banner 使用 observation-aware stop reason，区分 `observing-runtime`、`waiting-operator-gate`、`handoff-ready`、`recovery-attention`、`terminal`、`no-candidate`、`max-ticks` 和 `failed`，并隔离 task-scoped/global 展示。
...
- 当前验证结果：本轮已通过 `pnpm test`、`pnpm typecheck`、`pnpm --filter @coordinator/core build`、`openspec validate add-inner-coding-agent-lifecycle --strict`；上一轮 Web V2 验证的 `pnpm --filter @coordinator/db build`、`pnpm --filter @coordinator/api build`、`pnpm --filter @coordinator/web build`、`openspec validate --all --strict` 结论继续作为历史基线。真实 Web smoke 已走到 `implementation/implement`：`freeze-requirements` 和 `approve-planning-dossier` 在 inner final response evidence ready 后确认，`materialize-change` 未进入 needs-me，由 inner agent 创建并校验 `add-s1-signup-form-cache` 后推进。
- 当前 review 结论：Slice 17.1 独立 subagent review 未发现修改 workflow 工程/protocol schema、Web/daemon/outer 自动执行 workflow action 或 human gate、读写 `.workflow` private state、污染 Core/Daemon/Workflow protocol/AgentProvider/Web 分层，或把 raw provider events / hidden reasoning 变成状态机或 Surface。review 指出的 gate evidence 同秒 timestamp 边界、空 provider final response 占位文本、跨 workflow run evidence 污染风险已修复，并补充 event id / workflow-run-bound lifecycle event 回归测试。
- 当前 Slice 17.1 已落地：daemon 可在 active workflow run 中启动 SDK-backed inner coding agent，operator gate evidence ready 后才停给 Web 人工确认；`materialize-change <change-id>` 与 `continue-implementation` 保持 internal/debug，不进入 needs-me。真实 Web smoke 已验证 `freeze-requirements`、`approve-planning-dossier` evidence ready 后可确认，`materialize-change` 由 inner agent 自动创建并校验 `add-s1-signup-form-cache` 后调用 workflow action 推进；同时修复了 active workflow dirty workspace 被误标 operator attention、internal action 被旧 evidence 阻断、旧 evidence 同秒复用、空 final response 误提交和跨 run evidence 污染问题。
- 下一阶段：继续关注 Web V2 信息架构重做和 inner agent final response 在 Focus Drawer 中的 evidence 呈现质量。
...
### Iteration 17: Inner Coding Agent Lifecycle

本阶段必须继续遵守第 4 节固定动作。每个实现切片开始前必须阅读根目录 `AGENTS.md`、`docs/AGENTS.md`、`docs/workflow-agent-lifecycle-handoff.md`、`docs/execution-workspace.md`、`docs/workflow-protocol.md`、`docs/web-developer-workbench.md`、`docs/observability.md`、`docs/daemon.md` 与相关专题文档；创建 OpenSpec change；实现和测试；交给独立 subagent review，明确检查是否符合 `docs/` 总体设计心智、是否过度设计、是否污染 Core/Daemon/Workflow protocol/AgentProvider/Web 分层、是否错误让 daemon/outer Agent 自动执行 workflow action、是否把 raw provider events 或 hidden reasoning 变成新状态机。

#### Slice 17.1: SDK-backed inner coding agent lifecycle

OpenSpec change：

```text
add-inner-coding-agent-lifecycle
```

目标：

- Core 增加 `role=inner` coding agent runtime，复用现有 Codex / Claude SDK-first `AgentProvider`，但 cwd 指向 ready workspace 的 `repo/`。
- inner prompt 明确要求 coding agent 在 workflow 约束下推进，遇到 operator gate、handoff、失败或无法继续时输出 human-visible final response，形成 workflow gate evidence。
- daemon 在 active workflow run 需要 runtime/inner 继续处理、当前没有 active inner session、且当前不是 operator gate evidence 可提交时启动 inner coding agent；agent 运行中只观察，agent 完成后再通过 workflow protocol status inspect 对齐状态。
- operator detail / Web V2 展示 inner session activity、final response artifact refs 和 gate evidence 状态，但不扩大 Workflow Action Panel、Needs-Me 或 Coordinator Agent Surface。

边界：

- 不修改 `/Users/hetao/Documents/github/workflow` 或 workflow protocol schema。
- 不读取、解析或修改 `.workflow` private state。
- daemon、outer Agent、Web 都不能自动确认 human gate、workflow operator action 或 merge。
- `materialize-change`、`run-alignment-checks`、inspect/resume 和 unknown/debug action 仍不进入 needs-me。
- raw provider JSONL、hidden reasoning、permission internals 和完整 transcript 只保留为 debug artifact/ref，不作为 Core 状态机真源。

验收建议：

- 单测覆盖 inner cwd/permission profile/prompt 边界、active inner 防重、missing evidence 启动 inner、ready evidence 不自动确认、inner 完成后 workflow inspect。
- 单测覆盖 active workflow dirty workspace defer、internal action 不被旧 evidence 阻断、active inner lock/watchdog recovery。
- Web/model 测试覆盖 inner activity 可见、internal action 不进 needs-me、ready/missing evidence 的按钮状态不回归。
- 真实 Web smoke 尽量走到 `freeze-requirements`：inner final response 可见后，gate evidence 从 `missing` 变为 `ready`；确认后 workflow 离开 requirements 或进入下一停点；internal action 不要求开发者手填参数。
...
### Iteration 13: Web Developer Workbench 历史基线

Iteration 13 已完成第一版 Web operator surface、任务创建、Project Admin、Task Cockpit、Workflow Lens、PR/MR operator actions 和 `Run until blocked` 的基础落地。该阶段的设计以旧的 board/card/cockpit/debug 分层为主，为真实 Web smoke 和后续 workflow action loop 验证提供了必要基础。

从 Web V2 开始，旧 Iteration 13 不再作为新页面结构和视觉方向的实现基线：

- 旧 `Task Cockpit` 的主要信息迁移到 `Focus Drawer`。
- 旧侧栏 `Action Inbox` 收敛为 `Needs-Me Gate Inbox`，只展示真正 operator-facing gate。
- 旧 task card board 改为 `Run Matrix` 行式结构。
- 旧 `Classic Debug` 默认能力迁入 `Workflow Lens` / `Debug Detail Drawer`。
- 旧 `industrial mission control` 深色视觉方向被 `Operational Paper + Instrument Status` 取代。
- 后续新 Web 实现与验收以 `docs/web-developer-workbench.md` 的 Web V2 方案为准。

### Iteration 14: Agent lifecycle 与 SDK-first Provider Runtime

Iteration 14 的目标是把 Coordinator 从“轮询 workflow allowed action 的遥控器”收敛为“多个 workflow run / agent session 的生命周期管理者、观察者、恢复者和人工 gate 收件箱”。本迭代不修改 `/Users/hetao/Documents/github/workflow`，不要求 `workflow protocol` 立即新增字段；所有 protocol ownership 字段都只作为未来可选交接方向。

本阶段必须继续遵守第 4 节固定动作。每个实现切片开始前必须阅读根目录 `AGENTS.md`、`docs/AGENTS.md`、`docs/workflow-agent-lifecycle-handoff.md`、`docs/execution-workspace.md`、`docs/daemon.md`、`docs/workflow-protocol.md`、`docs/web-developer-workbench.md` 与相关专题文档；创建 OpenSpec change；实现和测试；交给独立 subagent review，明确检查是否让 daemon/outer Agent 自动执行 workflow action、是否污染 workflow/Core/AgentProvider 分层、是否把 SDK raw events 变成新状态机。

#### Slice 14.1: Align workflow agent lifecycle and Web operator gate

目标：

- 将 Web Workflow Action Panel 从 `allowedActions.length > 0` 调整为只展示 operator-facing gate。
- 在 Coordinator 侧定义保守 action classification：
  - operator-facing：`freeze-requirements`、`approve-planning-dossier`、`approve-review`、明确 approval/merge 类 gate。
  - agent/internal：`materialize-change`、`run-alignment-checks`、repair/current-change、inspect/resume、实现推进类 action。
  - unknown：默认 debug-only，不进入 needs-me。
- `materialize-change <change-id>` 不进入 Action Inbox，不要求 Web operator 填写 `change-id`，只在 Workflow Lens / debug detail 中展示 action input hint。
- `Run until blocked` 文案区分 `still running / observing`、`waiting operator gate`、`handoff ready`、`operator attention required`。
- Core operator action helper 在执行前除 latest allowed/denied/actionInputs/stateVersion 外，还校验 action classification 是 operator-facing。

边界：

- 不修改 workflow 工程或 workflow protocol。
- 不让 daemon 自动执行 agent/internal action。
- 不把 actionInputs 放入 Coordinator Agent Surface。
- 不把 Web 做成 workflow 内部遥控器。

验收建议：

- tests 覆盖 `freeze-requirements` 和 `approve-planning-dossier` 仍显示并可由 Core 校验执行。
- tests 覆盖 `materialize-change` 不进入 Action Panel/needs-me，API operator helper 拒绝非 operator-facing action 或 Web 不触达该 endpoint。
- tests 覆盖 daemon 仍只 inspect/reconcile，不自动执行 workflow action。
- 真实 Web smoke 确认 internal action 处不要求开发者手动输入参数。

#### Slice 14.2: SDK-first outer Coordinator Agent provider runtime

目标：

- 将 outer `CodexProvider` 和 `ClaudeCodeProvider` 从直接拼 CLI 参数升级为 SDK-first adapter：
  - Codex 使用 `@openai/codex-sdk`。
  - Claude 使用 `@anthropic-ai/claude-agent-sdk`。
  - 现有 CLI subprocess 保留为 compatibility fallback。
- 统一 provider session id、cwd/sessionRoot、permission profile、provider version、cancel/resume 和 transcript artifact。
- outer Coordinator Agent 继续保持 decision-only：cwd 固定 sessionRoot，权限最小，不直接写 repo/workspace。
- provider adapter 输出统一 `AgentProviderRunResult`，并开始保留 raw provider events artifact。

边界：

- 不改变 Coordinator Agent Surface / tools。
- 不让 SDK provider 绕过 Core tool executor。
- 不把 SDK private session 文件或 raw JSONL 当作 Core 真相源。
- 不在本切片改造 inner coding agent ownership。

验收建议：

- contract tests 覆盖 SDK adapter 的启动、final response、session id、cwd、permission profile、fallback 和 failure。
- tests 断言 outer provider cwd 仍是 sessionRoot，不指向 project repo 或 workspace repo。
...
- 建立 provider event 三层模型：
  - raw provider events -> `provider-events.jsonl` artifact。
  - normalized agent events -> timeline/Web 摘要。
  - decision signals -> 极少 lifecycle 字段，例如 `agent.state`、`last_activity_at`、failure kind。
- Web 展示 agent running、last activity、latest normalized event、final response artifact，而不是默认展示完整 transcript。
- daemon/watchdog 使用 lifecycle signal 做探活、stalled 判断和 recovery observation，但不让 event stream 驱动业务完成。

边界：

- raw event 不进入 Coordinator Agent prompt。
- raw event 不驱动 PR readiness、done、merge 或 workflow handoff。
- 不把 provider raw output、secret、permission internals、完整 JSONL 暴露到 agent-facing surface。

验收建议：

- tests 覆盖 raw event artifact、normalized event 白名单、agent lifecycle 更新、surface 不泄漏 raw event。
- Web build/typecheck 通过，任务详情能看到 agent activity 摘要。
...
- 在不修改 workflow protocol 的前提下，Coordinator 侧先用保守策略表达 inner agent lifecycle：
  - workflow active + no handoff + only internal allowed action => observing / waiting runtime，不是 needs-me。
  - workflow active + operator-facing gate => Action Panel。
  - workflow handoff available => PR/MR/review/merge flow。
  - provider/workflow inconsistent or failed => recovery/operator attention。
- 整理一份给 workflow 工程的未来交接建议，说明后续可增加 `agent.state`、`blocker.owner`、`operatorActions`、`agentActions`、`lastAgentEventSummary`，但不作为当前 Coordinator 实现前提。
- 如后续 workflow 已提供这些字段，Coordinator 再单独适配；本切片只保证现有 protocol 下不误打断开发者。

边界：

- 不读取 `.workflow` private state。
- 不让 Coordinator 猜 `change-id`。
- 不让 daemon/outer Agent 自动调用 `materialize-change`。
...
### Iteration 15: Web Developer Workbench V2

Iteration 15 的目标是把 Web 从 Iteration 13 的旧 board/card/cockpit 结构重构为桌面端 `Run Matrix + Focus Drawer + Unified Composer`。本阶段以 `docs/web-developer-workbench.md` 为唯一产品与视觉基线，不考虑移动端或手机屏幕适配。

本阶段必须继续遵守第 4 节固定动作。每个实现切片开始前必须阅读根目录 `AGENTS.md`、`docs/AGENTS.md`、`docs/web-developer-workbench.md`、`docs/workflow-agent-lifecycle-handoff.md`、`docs/workflow-protocol.md`、`docs/daemon.md`、`docs/coordinator-surface.md`、`docs/observability.md` 与相关专题文档；创建 OpenSpec change；实现和测试；交给独立 subagent review，明确检查是否符合 `docs/` 总体设计心智、是否过度设计、是否污染 Core/Daemon/Workflow protocol/AgentProvider/Web 分层、是否让 daemon 或 outer Agent 自动执行 workflow action、是否把 raw provider events 或 workflow debug 字段变成新状态机。

本阶段拆成 3 个切片，尽量少但保持可验证和可回退。

#### Slice 15.1: Web V2 shell, Run Matrix and Focus Drawer

建议 OpenSpec change：

```text
rebuild-web-workbench-v2-shell
```

目标：

- 从头建立新的桌面端 Web V2 单页 shell：Command Bar、Run Matrix、Focus Drawer、Unified Composer 占位、Debug Detail 的基础布局。
- 用 Run Matrix 替代旧 task card board；每个 task 一行，展示 project/title、outer lifecycle rail、workflow stage rail、stage/substate chip、owner/mode、heartbeat、needs-me pin 和 focus action。
- 用 Focus Drawer 承接旧 Task Cockpit 的主要信息：task summary、workflow run summary、agent activity summary、operator gate panel、recent evidence、attachments shelf、Workflow Lens 入口。
- 删除旧 Workbench board、Task Cockpit、Classic Debug、New Task、Project Admin 页面结构；必要 operator/debug 能力只保留在 Web V2 单页的 Focus Drawer / Debug Detail 中。
- 使用 `Operational Paper + Instrument Status` 视觉方向：浅色、高密度、细线轨道、克制状态色和状态驱动动效。
...
- 不读取 `.workflow` private state。
- 不把 `allowedActions.length > 0` 作为 needs-me 触发条件。
- 不做移动端适配；桌面视口是验收目标。

验收建议：

- Web 默认首页是 Run Matrix，不是单 task debug 页面或 marketing/landing 页面。
- 多 task 行能同时展示外层 lifecycle rail 和 workflow stage/substate rail。
- 选中 task 后 Focus Drawer 展示当前 workflow/agent/operator gate 摘要。
- Debug Detail 默认折叠，raw timeline/surface/operation/provider event 不默认占据主界面。
- `data-task-id`、主要按钮 `aria-label` 和稳定测试定位已补齐。
- Browser/Playwright 截图验证桌面视口无重叠、无明显文本溢出、轨道尺寸稳定。
- `pnpm --filter @coordinator/web build`、`pnpm typecheck` 和相关 tests 通过。

#### Slice 15.2: Gate Inbox, lifecycle observation and run-until-blocked UX

建议 OpenSpec change：

```text
align-web-v2-gate-inbox-runtime-observation
```

目标：

- 将旧 Action Inbox 收敛为 `Needs-Me Gate Inbox`：只展示 pending human request、merge approval、PR/MR review/conflict、operator-facing workflow gate、project/provider blocker 和 Core recovery attention。
- Web V2 全面使用 Core 提供的 workflow action classification 与 runtime observation；`materialize-change <change-id>`、alignment checks、inspect/resume、实现推进类 action 只进入 Workflow Lens / Debug Detail。
- 调整 Run Matrix row、Focus Drawer 和 `Run until blocked` banner 的 owner/mode 表达，区分 `observing-runtime`、`waiting-operator-gate`、`handoff-ready`、`recovery-attention`、`terminal`、`no-candidate`。
- task-scoped run banner 只显示当前 task 的结果；global run 结果不污染当前 Focus Drawer。
- 真正 operator gate 的 action 继续通过 API/Core runtime 提交，Core 校验 stateVersion、classification、allowed/denied/actionInputs 和 operation/idempotency。

边界：

- Web 不直接执行 workflow CLI。
- Web 不自动执行 workflow action。
- daemon/outer Agent 不自动确认 human gate。
- stage/substate 只用于展示，不推导 PR readiness、done 或 merge。
- raw SDK/provider event 只作为 observation 摘要和 artifact ref，不驱动业务完成。

验收建议：

- tests 覆盖 agent/internal action 不进入 Needs-Me Gate Inbox。
- tests 覆盖 operator-facing workflow gate 仍可在 Focus Drawer 中确认，并由 Core 受控调用 workflow action。
- tests 覆盖 `Run until blocked` 在 internal action 或 running inner agent 场景显示 observing，不要求开发者填内部参数。
- 真实 Web smoke 至少走到 `materialize-change <change-id>`，确认它不进入 needs-me。
- `pnpm --filter @coordinator/web build`、相关 Core/API tests、`pnpm typecheck` 通过。

#### Slice 15.3: Unified Composer, local attachments and real-flow hardening

建议 OpenSpec change：

```text
add-web-v2-composer-attachments-real-flow
```

目标：

- 实现 Unified Composer：未选 task 时创建新 task；选中 task 且存在 pending human request/operator gate 时作为回复/确认入口；选中 task 且无 pending gate 时作为 task note 或 follow-up context。
- 实现本地附件第一版：Web 上传图片/文件，API 保存到 Coordinator 管控的 task attachment root，DB 记录 metadata，operator summary/task brief 暴露 artifact refs；workspace/agent 场景通过受控路径或 refs 可见。
- 附件路径遵守 artifact root 和 path containment，不直接写 project repo 根目录，不读写 `.workflow` private state，不把大附件内容塞进 Coordinator Surface prompt。
- 收敛旧 New Task 页面和旧 task note/answer 入口到 Composer；保留必要 Project Admin 入口，但不在本切片落地 workspace hook 副作用。
- 做真实 Web flow hardening：新建 task、上传附件或安全占位、run until blocked、观察 workflow stage/substate、确认 operator gate、进入 handoff/PR/MR 或下一停点。

边界：

- 不实现移动端。
- 不执行 workspace init/cleanup 任意脚本。
- 不修改 workflow protocol。
- 不把附件内容变成 Core 状态判断依据。
- 不扩大 Coordinator Agent Surface；复杂信息仍优先写 artifact，并通过 refs 暴露。

验收建议：

- Composer 在三类上下文下行为正确：新建 task、回复 gate、追加上下文。
- 附件有 size/type/path/cleanup 或 retention 的最小 contract 与 tests。
- agent/task summary 只看到附件 refs 和受控路径，不看到大块二进制内容或 raw file payload。
- 真实 Web smoke 尽量走到最远，并记录是否进入 requirements gate、implementation/internal action、handoff 或 recovery attention。
- Browser/Playwright 截图验证桌面视口下 Composer、attachments shelf、Focus Drawer 无重叠。
