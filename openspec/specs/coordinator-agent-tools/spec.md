...
## Purpose
定义 `coordinator` 第一版 Coordinator Agent Tools executor 契约：系统必须让外层 Coordinator Agent 只能通过当前 Coordinator Surface 暴露的受控工具推进 execution plan、attempt/workspace、workflow run 和 human request，并记录可观测 tool trace。该规格不包含 daemon、PR/MR provider、merge approval 或 agent provider 自动 tool-call 解析。
## Requirements
