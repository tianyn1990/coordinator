# pr-mr-merge-reconciliation Specification

## Purpose

定义 PR/MR 与 merge 的恢复矩阵：系统必须把 provider 返回的有限外部事实交给 Coordinator Core，由 Core 决定 approval invalidation、operation reconciliation、merge race/conflict 和 operator attention。
