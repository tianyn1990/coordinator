...
定义 `Coordinator Surface` 的第一版契约：系统必须把 Core 中的机器状态翻译成同源的 machine JSON surface 与 agent-facing Markdown surface，并按当前状态收窄 agent tools。该规格只覆盖 Iteration 4，不包含真实 agent tool 执行、daemon、workspace manager、workflow adapter 或 PR/MR provider。
